...
class V3:
    mappings = {'one': ga.throttle, 'stop': ga.brake}
    turning_angle = 30
    ...
    def mark_flags(flag_arr, gesture):
        ...
        """
        if gesture in V3.mappings.keys():
            flag_arr[gesture] = 1
    ...
    def sim_input(gamepad, flags):
        ...
        """
        for key in flags:
            V3.mappings[key](key, flags, gamepad)
            if gamepad:
                gamepad.update()
    ...
    def run(detector, threshold=0.5):
        ...
        t1 = 0
        while cap.isOpened():
            gesture_flags = {key: 0 for key in V3.mappings.keys()}
            delta = (time.time() - t1)
            t1 = time.time()

            ret, frame = cap.read()
            if ret:
                processed_frame, size, padded_size = V3.preprocess(frame)
                with torch.no_grad():
                    ...
                    if results.multi_hand_landmarks:
                        ...
                        if len(results.multi_hand_landmarks) >= 2:
                            ...
                            if results.multi_hand_landmarks[0].landmark[0].x < results.multi_hand_landmarks[1].landmark[
                                0].x:
                                lm1 = (int(results.multi_hand_landmarks[0].landmark[16].x * w),
                                ...
                                lm1 = (int(results.multi_hand_landmarks[1].landmark[16].x * w),
                                       int(results.multi_hand_landmarks[1].landmark[16].y * h))
                ...
                for i in range(min(2, len(boxes))):
                    if scores[i] > threshold:
                        ...
                        V3.mark_flags(gesture_flags, hand_gesture)

                V3.sim_input(gp, gesture_flags)
                fps = 1 / delta
